"""
Test script for RHMC with LGCP AR(p) model.

This benchmark uses:
- AR(p) latent dynamics with self-regulation: x_{t+1} = μ + Σφᵢ(x_{t-i+1}-μ) - β exp(x_t) + σε_t
- Poisson observations: y_t ~ Poisson(exp(b + x_t))

The model creates challenging geometry because:
- Self-regulation -β exp(x) creates state-dependent dynamics Jacobian
- Poisson Fisher information exp(η) creates state-dependent observation curvature
- Both scale with exp(x), creating correlated curvature changes
- Near-persistent dynamics (Σφᵢ ≈ 1) create long-range correlations

Configuration:
- p = AR order (configurable)
- K = total time steps
- Parameters: φ₁,...,φₚ (AR coeffs), μ (drift), β (self-regulation), b (intercept)
"""
...
μ_true = 0.0        # Drift
β_true = 0.05       # Self-regulation strength (moderate)
b_true = 1.0        # Observation intercept (baseline rate ~ exp(1) ≈ 2.7)

# θ = [φ₁, ..., φₚ, μ, log(β), b]
θ_true = SVector{P + 3,Float64}(φ_true..., μ_true, log(β_true), b_true)

# Create model
model = LGCPARpModel{P}(; σ=σ, s0=1.0)
...
println("Model: LGCPARpModel (AR($P) + self-regulation)")
...
println("\nSimulating LGCP with AR($P) dynamics...")
...
p1 = plot(;
    title="Latent Log-Intensity x_t (AR($P) + self-regulation)",
    xlabel="Time step",
    ylabel="x_t",
    legend=:topright,
    size=(900, 400),
...
# θ = [φ₁, ..., φₚ, μ, log(β), b]
prior_mean = SVector{P + 3,Float64}(ntuple(i -> i <= P ? 0.3 : 0.0, Val(P + 3)))
prior_var = SVector{P + 3,Float64}(ntuple(_ -> 4.0, Val(P + 3)))
...
initial_θ_params = θ_true .+ 0.1 * randn(rng, P + 3)
...
β_samples = [exp(s[param_offset + P + 2]) for s in samples]
b_samples = [s[param_offset + P + 3] for s in samples]
...
println("log(β) ESS: ", param_ess[P + 2])
println("b ESS: ", param_ess[P + 3])
...
println(
    "β: true=$(β_true), posterior mean=$(round(mean(β_post), digits=4)), std=$(round(std(β_post), digits=4))",
)
println(
...
β_samples_hmc = [exp(s[param_offset + P + 2]) for s in hmc_samples_raw]
b_samples_hmc = [s[param_offset + P + 3] for s in hmc_samples_raw]
...
println("log(β) ESS: ", hmc_param_ess[P + 2])
println("b ESS: ", hmc_param_ess[P + 3])
...
println(
    "β: true=$(β_true), posterior mean=$(round(mean(β_post_hmc), digits=4)), std=$(round(std(β_post_hmc), digits=4))",
)
println(
...
println("\n" * "="^60)
println("COMPARISON SUMMARY: RHMC vs HMC for LGCP AR($P)")
println("="^60)
...
println(
    "μ=$(round(param_ess[P + 1], digits=4)), β=$(round(param_ess[P + 2], digits=4)), b=$(round(param_ess[P + 3], digits=4))",
)
...
println(
    "μ=$(round(hmc_param_ess[P + 1], digits=4)), β=$(round(hmc_param_ess[P + 2], digits=4)), b=$(round(hmc_param_ess[P + 3], digits=4))",
)
...
plot!(
    p_trace[P + 2], β_samples_hmc; subplot=P + 2, label="HMC", lw=1, color=:green, alpha=0.5
)
hline!(p_trace[P + 2], [β_true]; subplot=P + 2, label="True", lw=2, color=:red)
...
plot!(
    p_trace[P + 3], b_samples_hmc; subplot=P + 3, label="HMC", lw=1, color=:green, alpha=0.5
)
hline!(p_trace[P + 3], [b_true]; subplot=P + 3, label="True", lw=2, color=:red)
...
p_traj = plot(;
    title="Posterior x_t Trajectories",
    xlabel="Time step",
    ylabel="x_t",
    legend=:topright,
    size=(900, 400),
