...
This version implements caching for improved performance during leapfrog integration.

This file demonstrates how to connect the fast block tridiagonal metric tensor computations
to AdvancedHMC's Riemannian HMC implementation with caching optimization.
"""
...
This cached version allows reusing θ-dependent computations.
"""
...
"""
Cached version of ∂H∂θ for improved performance.

Terms that only depend on θ can be cached and reused when θ doesn't change
(e.g., during momentum updates in leapfrog integration).
"""
function AdvancedHMC.∂H∂θ_cache(
    ...
    if isnothing(cache)
        ...
        ℓπ, trace_grad, G_chol, dGs = cache
    end

    # Convert momentum to block format (depends on r, must be recomputed)
    r_blocks = to_block_vector(r, Val(D))

    # Compute G^{-1} * r using efficient block operations (depends on r)
    G_inv_r = G_chol \ r_blocks

    # Compute gradient contributions
    # Start with cached trace contributions, then add quadratic form terms
    grad = copy(trace_grad)
    ...
    return return_cache ? (dv, (; ℓπ, trace_grad, G_chol, dGs)) : dv
end
