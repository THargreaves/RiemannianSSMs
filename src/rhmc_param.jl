...
struct BorderedBlockTridiagonalRiemannianMetric{T,D,P,SSM,YT} <:
...
function BorderedBlockTridiagonalRiemannianMetric(
    ...
    T = eltype(prior_mean)
    return BorderedBlockTridiagonalRiemannianMetric{T,D,P,typeof(ssm),typeof(ys)}(
        ssm, ys, K, prior_mean, prior_prec
    )
end
...
function calc_G(
    ...
    return calc_G_param(z_blocks, metric.ssm, metric.prior_prec; λ=λ)
end
...
function AdvancedHMC.∂H∂θ_cache(
    ...
    if isnothing(cache)
        z_blocks = to_bordered_block_vector(z, Val(D), Val(P))
        ℓπ = calc_ll_param(
            z_blocks, metric.ys, metric.ssm, metric.prior_mean, metric.prior_prec
        )
        ∂ℓπ∂z = calc_ll_grad_param(
            z_blocks, metric.ys, metric.ssm, metric.prior_mean, metric.prior_prec
        )

        G = calc_G_param(z_blocks, metric.ssm, metric.prior_prec)
        G_inv_parts = bordered_block_selected_inv(G)
        G_chol = cholesky(G)
        dGs_state = calc_dGs_state(z_blocks, metric.ssm)
        dGs_param = calc_dGs_param(z_blocks, metric.ssm)
...
function calc_G_param(
    z::BorderedBlockVector{T,D,P}, ssm, prior_prec::SVector{P,T}; λ=1e-7
) where {T,D,P}
    K = length(z.state_blocks)
    θ = z.param_block
...
"""
function calc_dGs_state(z::BorderedBlockVector{T,D,P}, ssm) where {T,D,P}
    K = length(z.state_blocks)
    θ = z.param_block
...
"""
function calc_dGs_param(z::BorderedBlockVector{T,D,P}, ssm) where {T,D,P}
    K = length(z.state_blocks)
    θ = z.param_block
    ...
    for p in 1:P
        ...
        @inbounds for k in 1:K
            ...
            # Corner block contributions from this k
